...
`;
/* ================= COMPONENT ================= */
export function ImportCoursesDialog({ open, onOpenChange, onImport, }) {
    ...
    return (_jsx(AlertDialog, { open: open, onOpenChange: onOpenChange, children: _jsxs(AlertDialogContent, { className: "max-w-lg space-y-4", children: [_jsx(AlertDialogHeader, { children: _jsx(AlertDialogTitle, { children: "Import Courses (CSV)" }) }), _jsxs("div", { className: "space-y-2", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsx("span", { className: "text-sm font-medium", children: "ChatGPT Prompt (use with timetable image)" }), _jsx(Button, { size: "sm", variant: "outline", onClick: async () => {
                                        await navigator.clipboard.writeText(CHATGPT_PROMPT);
                                        toast.success("Prompt copied to clipboard");
                                        setCopied(true);
                                        setTimeout(() => setCopied(false), 15000);
                                    }, children: copied ? "Copied!" : "Copy" })] }), _jsx(Textarea, { readOnly: true, rows: 8, value: CHATGPT_PROMPT, className: "text-xs font-mono" })] }), _jsx(Textarea, { rows: 6, placeholder: "name,color,day,sessions", value: csvText, onChange: (e) => setCsvText(e.target.value), className: "font-mono" }), _jsxs(AlertDialogFooter, { children: [_jsx(AlertDialogCancel, { children: "Cancel" }), _jsx(Button, { onClick: parseCSV, children: "Run" })] })] }) }));
}
